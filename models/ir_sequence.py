...
www.sii.cl'''.format(folio)
        ...
        for caffile in caffiles:
            if int(folio) >= caffile.start_nm and int(folio) <= caffile.final_nm:
                if caffile.expiration_date and not caffile.sequence_id.sii_document_class_id.es_boleta():
    ...
    def _next_do(self):
        ...
        if self.sii_document_class_id and self.forced_by_caf and self.dte_caf_ids:
            ...
            if number_next +1 != actual: #Fue actualizado
                number_next = actual
                # llamar a la funcion super para que se avance a la siguiente secuencia
                # ya que en un cambio de CAF, se toma el primer folio, pero la secuencia no avanza al proximo
                # lo que provoca que la siguente vez se vuelva a dar el mismo folio
                super(IRSequence, self)._next_do()
