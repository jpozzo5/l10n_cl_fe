...
class ColaEnvio(models.Model):
    ...
    number_document = fields.Char("N° Documento", compute="_get_name_document")
    
    def _get_name_document(self):
        logging.info(self)
        for field in self:
            ID_document = int(field.doc_ids.replace("]","").replace("[",""))
            logging.info(field.model)
            factura = self.env[field.model].search([('id', '=', ID_document)])
            if field.model == "account.invoice":
                if factura.sii_document_number:
                    field.number_document = factura.sii_document_number
    ...
    def _procesar_tipo_trabajo(self):
        ...
        if (self._es_doc(docs[0]) or (self.es_boleta(docs[0]) and \
            ...
            if self.send_email and docs[0].sii_result in ['Proceso', 'Reparo']:
                """
                    En realidad el documento aca esta en consulta,siempre y cuando sea boleta,por eso
                    no entra en la proxima condicion.
                """
                for doc in docs:
                    ...
                    if not doc.sii_xml_request.sii_receipt: 
                        resp_b = doc.consultar_boleta()#Metodo nuevo Realiza la consulta al sii el status de la boleta nuevo webservice
                        doc.sii_xml_request.sii_receipt = resp_b.decode("utf-8")
                        resp_json = eval(resp_b.decode("utf-8"))
                        if not 'error' in resp_json:
                            self.enviar_email(doc)#aca lo que hace es enviarle el Dte al cliente
                            self.unlink()
                            return
                    else:
                        res = json.loads(doc.sii_xml_request.sii_receipt)
                        resp_b = doc.consultar_boleta()#Metodo nuevo Realiza la consulta al sii el status de la boleta nuevo webservice
                        doc.sii_xml_request.sii_receipt = resp_b.decode("utf-8")
                        return
