...
class AccountInvoice(models.Model):
    ...
    def _default_journal_document_class_id(self):
        ...
        jdc = self.env['account.journal.sii_document_class'].search([
            ('journal_id', '=', journal),
            ('sii_document_class_id.sii_code','=',39),
    ...
    def _get_default_document(self):
        document = self.env['sii.document_class'].search([('sii_code','=',39)])
        if document:
            return document.id
        else:
            return 4
    ...
    document_class_id = fields.Many2one(
            ...
            readonly=True,
            states={'draft': [('readonly', False)]},
            default=lambda self: self._get_default_document(),  
    )
...
a VAT."))
    ...
    def _totales(self, MntExe=0, no_product=False, taxInclude=False):
            ...
            if not self._es_boleta() or  not taxInclude or self._context.get('tax_detail'):
                ...
                for t in self.tax_line_ids:
                    if t.tax_id.sii_code  == 14:
                        ...
                        break
                MntNeto = self.amount_total - self.amount_tax
                if self._es_boleta():
                    IVA = self.amount_tax #esto lo hago para que pueda tomar el monto iva en la libreria
                    MntIVA = self.amount_tax

                exp = any([ True if t.tax_id.sii_code in [26, 24] else False for t in self.tax_line_ids])
        ...
        if  not self._es_boleta() or not taxInclude or self._context.get('tax_detail'):#not self._es_boleta() or
            if IVA:
                if not self._es_boleta() or self._context.get('tax_detail'):#not self._es_boleta() or
        ...
        MntTotal = sum([line.price_total for line in self.invoice_line_ids])
        if no_product:
            MntTotal = 0
...
#                lines['RUTMandante']
            ...
            if 'IndExe' in lines:
                taxInclude = False
            if not self._es_boleta():
                lines['PrcItem'] = round(lines['PrcItem'] / (1 + (sum(totalImp)/100)),2)
                lines['MontoItem'] = round(lines['PrcItem'] * line.quantity,2)
            else:
                lines['PrcItem'] = round(lines['PrcItem'],2)
                lines['MontoItem'] =round(lines['PrcItem'] * line.quantity,2)

            #logging.info(lines['PrcItem'] )
            logging.info("despacho de lineas")
    ...
    def _timbrar(self, n_atencion=None):
        ...
        #logging.info(result[0]['sii_xml_request'])
        self.write({
    ...
    def _get_dte_status(self,type_document = None):
        if type_document == 39:
            datos = self[0]._get_datos_empresa(self[0].company_id)
            datos['Documento'] = []
            docs = {}
            ...
            for r in self:
                ...
                r.sii_result = resultself.ado[id]['status']
    ...
    def ask_for_dte_status(self):
        for r in self:
            ...
            try:
                self._get_dte_status(r.document_class_id.sii_code)
            except Exception as e:
    ...
    def get_token_boleta(self,seed):
        url = 'https://api.sii.cl/recursos/v1/' + 'boleta.electronica.token'
        seed = u'<?xml version="1.0" encoding="UTF-8"?>' + seed
        req = pool.request('POST', url, body=seed, headers={
            'Accept': "application/xml",
            "Content-Type": "application/xml"})
        resp = req.data.decode('UTF-8')
        root = etree.fromstring(resp.replace('<?xml version="1.0" encoding="UTF-8"?>', ''))
        return root[0][0].text


    def consultar_boleta(self ):
        response = None
        for field in self:   
            res = json.loads(field.sii_xml_request.sii_xml_response)
            tackID = res['trackid']
            rut = int(res['rut_emisor'].split('-')[0])
            dv = int(res['rut_emisor'].split('-')[1])
            semilla = self.get_semilla_boleta()
            token = self.get_token_boleta(semilla)
            try:
                ...
                raise UserError("Ha ocurrido un error al consultar el estado del documento.")
    ...
    def send_exchange(self):
        ...
        for dte_email in dte_receptors:
            if not dte_email.dte_email or not dte_email.email:
                continue
