...
""")
    ...
    def _find_current_document(self, journal, sii_document_class, sequence_name):
        return self.env['account.journal.sii_document_class'].search([
                ('sii_document_class_id', '=', sii_document_class.id),
                ('sequence_id.name', '=', sequence_name),
    ...
    def create_journal_document(self, letter_ids, document_type, journal):
        ...
        for document_class in document_class_ids:
            sequence_name = journal.name + ' - ' + document_class.name
            if self._find_current_document(journal, document_class, sequence_name):
                continue
