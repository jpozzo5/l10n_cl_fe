...
class UploadXMLWizard(models.TransientModel):
    ...
    def confirm(self, ret=False):
        ...
        if created:
            ...
            ctx.update({
                ...
                'active_ids': created,
                'active_id': created[0] if created else False,
            })
            result['context'] = ctx
        return result
    ...
    def _read_xml(self, mode="text", check=False, xml=None):
        if xml is None:
            xml = self._get_xml()
        xml = xml\
    ...
    def do_receipt_deliver(self):
        ...
        if self.dte_id:
            for r in respuesta:
                ...
                send_mail = self.env['mail.mail'].sudo().with_context(skip_xml_dte=True).create(values)
                send_mail.send()
    ...
    def _prepare_partner_vals(self, data):
        ...
        document_number = (re.sub('[^1234567890Kk]', '', data.find(rut_path).text)).zfill(9).upper()
        ...
        cities = self.env['res.city'].search([
                    ('name', '=',  data.find('Cmna%s' % dest).text.title())], limit=1)
        partner_vals = {
            'name': name,
            ...
            'street': data.find('Dir%s' % dest).text if not data.find('Dir%s' % dest) is None else '',
            'city': data.find('Ciudad%s' % dest).text if not data.find('Ciudad%s' % dest) is None else '',
            'city_id': cities.id,
            'state_id': cities.state_id.id,
            'country_id': cities.country_id.id,
            'phone': data.find('Telefono').text if not data.find('Telefono') is None else '',
            'company_type': 'company',
        }
        if cities:
            partner_vals['city'] = cities.name
        if self.type == 'compras' and data.find('CorreoEmisor') is not None:
            partner_vals.update({
                'email': data.find('CorreoEmisor').text,
                'dte_email': data.find('CorreoEmisor').text,
            })
        elif self.type == 'ventas' and data.find('CorreRecep') is not None:
            partner_vals.update({
                'email': data.find('CorreoRecep').text,
                'dte_email': data.find('CorreoRecep').text,
            })
        if self.type == 'compras':
            partner_vals.update({'supplier': True})
        return partner_vals

    def _create_partner(self, data):
        if self.pre_process and self.type == 'compras':
            return False
        partner_vals = self._prepare_partner_vals(data)
    ...
    def _buscar_impuesto(self, name="Impuesto", amount=0, sii_code=0,
                         sii_type=False, IndExe=False, price_included=False, company_id=False):
        query = [
            ('amount', '=', amount),
            ('sii_code', '=', sii_code),
        ...
        if not imp:
            if price_included and self.env.context.get('raise_error'):
                raise UserError('No se encuentra un impuesto configurado como Incluido en el precio, por favor cree uno')
    ...
    def _get_price_product(self, line, price_included, tipo_dte):
        price = float(line.find("PrcItem").text if line.find("PrcItem") is not None else line.find("MontoItem").text)
        if price_included:
            IndExe = not line.find("IndExe") is None
            amount = 0
            sii_code = 0
            sii_type = False
            if tipo_dte in (34, '34'):
                IndExe = True
            if not IndExe:
                amount = 19
                sii_code = 14
                sii_type = False
            imp = self._buscar_impuesto(amount=amount, sii_code=sii_code, sii_type=sii_type, IndExe=IndExe, price_included=price_included)
    ...
    def get_product_values(self, line, company_id, price_included, tipo_dte):
        ...
        price = self._get_price_product(line, price_included, tipo_dte)
        values = {
    ...
    def _create_prod(self, data, company_id, price_included, tipo_dte):
        product_id = self.env['product.product'].create(self.get_product_values(data, company_id, price_included, tipo_dte))
        return product_id
    ...
    def _prepare_line(self, line, document_id, account_id, type, company_id, fpos_id,
        ...
        if self.pre_process and self.type == 'compras':
            ...
            ail_account = self.env['account.invoice.line'].get_invoice_line_account(type, product_id, fpos, company_id)
            data.update({
                'invoice_line_tax_ids': [(6, 0, product_id.supplier_taxes_id.ids)],
                'uom_id': product_id.uom_id.id,
                'price_unit': price,
                'account_id': ail_account.id
            })
            IndExe = not line.find("IndExe") is None
            amount = 0
            sii_code = 0
            ...
            if not IndExe:
                ...
                sii_code = 14
                sii_type = False
            # siempre forzar el IVA sin incluir impuestos
            # solo se pasa la variable price_included para calcular el precio neto
            imp = self._buscar_impuesto(amount=amount, sii_code=sii_code,
                                        sii_type=sii_type, IndExe=IndExe,
                                        price_included=price_included,
                                        company_id=company_id)
            if imp:
                if imp != product_id.supplier_taxes_id:
                    product_id.write({'supplier_taxes_id': [(6, 0, imp.ids)]})
    ...
    def _prepare_ref(self, ref):
        ...
        if not tpo:
            _logger.warning("No se encontro un tipo de documento con codigo: %s", TpoDocRef)
            return False
    ...
    def _prepare_invoice(self, documento, company_id, journal_id):
        ...
        if not partner_id:
            ...
            partner_vals = self._prepare_partner_vals(Encabezado.find('%s' % type))
            partner_id.sudo().write(partner_vals)
        if partner_id and not partner_id.supplier and self.type == "compras":
        ...
        if date_due is not None:
            invoice['date_due'] = date_due.text
        if journal_id:
    ...
    def _get_invoice_lines(self, documento, document_id, account_id, invoice_type, fpos,
        ...
        for line in documento.findall("Detalle"):
            new_line = self._prepare_line(line, document_id, account_id, invoice_type,
                                          company_id, fpos, price_included, tipo_dte)
            if new_line:
    ...
    def _get_data(self, documento, company_id, ignore_journal=False):
        Encabezado = documento.find("Encabezado")
        IdDoc = Encabezado.find("IdDoc")
        tipo_dte = IdDoc.find("TipoDTE").text
        price_included = IdDoc.find("MntBruto") is not None
        ...
        if Encabezado.find('Totales/ImptoReten') is not None:
            ...
            for i in ImptoReten:
                imp = self._buscar_impuesto(
                                ...
                                sii_code=i.find('TipoImp').text, price_included=price_included)
        ...
        if not self.pre_process and Referencias:
            ...
            for ref in Referencias:
                reference_vals = self._prepare_ref(ref)
                if reference_vals:
                    refs.append(reference_vals)
    ...
    def _create_inv(self, documento, company_id):
        ...
        for t in inv.tax_line_ids:
            ...
            elif Totales.find('MntExe') is not None:
    ...
    def _create_pre(self, documento, company_id):
        ...
        if dte:
            if self.env.context.get('raise_error'):
                raise UserError(_("El documento %s %s ya se encuentra registrado" % (dte.number, dte.document_class_id.name)))
    ...
    def _get_dtes(self, xml_file=None):
        xml = self._read_xml('etree', xml=xml_file)
        if xml.tag == 'SetDTE':
    ...
    def do_create_pre(self):
        ...
        for dte in dtes:
            try:
                ...
                if not company_id and self.env.user.company_id.dte_service_provider == 'SIICERT':
                    company_id = self.env.user.company_id
                if not company_id:
                    if self.env.context.get('raise_error'):
                        raise UserError("No existe compañia para %s" % documento.find("Encabezado/Receptor/RUTRecep").text)
                ...
                if pre:
                    # actualizar la company en mail.message.dte.document
                    # ya que al crearse desde una tarea cron, tomara la company del usuario admin
                    # pero no necesariamente es la company correcta
                    # y mail.message.dte tendra una company y mail.message.dte.document tendra otra company
                    # lo cual es incorrecto
                    pre.dte_id.sudo().write({
                        'company_id': company_id.id,
                    })
                    inv = self._inv_exist(documento)
                    pre.write({
        ...
        for dte in dtes:
            try:
                ...
                if self.type == 'ventas' or self.option == 'accept':
                    ...
                    if self.env.context.get('auto_validate_invoice'):
    ...
    def prepare_purchase_line(self, line, document_id, date_planned,
                              company_id, price_included, tipo_dte):
    ...
    def _create_po(self, documento, company):
        ...
        if not partner:
            ...
            partner_vals = self._prepare_partner_vals(Encabezado.find('Emisor'))
            partner.write(partner_vals)
        if partner and not partner.supplier:
