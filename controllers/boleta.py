...
class Boleta(http.Controller):
    ...
    def view_document(self, folio=None, **post):
        ...
        # pasar los datos del reporte cuando sea desde account.invoice, no desde el pos
        if orders and orders[0]._name == 'account.invoice':
            values.update(request.env['report.account.report_invoice']._get_report_values(orders.ids))
        return request.render('l10n_cl_fe.boleta_layout', values)

    def _get_report(self, document):
        return request.env.ref('account.account_invoices_without_payment').sudo().render_qweb_pdf([document.id])[0]
